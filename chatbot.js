...
function showChatbot() {
    ...
    chatbot.innerHTML = `
        <div id="chatbot-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <h4 style="margin: 0; font-size: 16px;">সাহায্য প্রয়োজন?</h4>
            <button id="minimize-btn" style="background: none; border: none; font-size: 18px; color: #007bff; cursor: pointer; padding: 0;">-</button>
        </div>
        <div id="chatbot-content">
            <p style="margin-bottom: 10px;">আপনি একটি অ্যাপয়েন্টমেন্ট বুক করতে চান?</p>
            <button onclick="openAppointmentForm()" style="background-color: #007bff; color: white; padding: 8px 12px; border: none; border-radius: 4px; cursor: pointer; transition: background-color 0.3s ease; margin-right: 5px;">হ্যাঁ, এখনই বুক করুন</button>
            <button onclick="closeChatbot()" style="background-color: #e0e0e0; color: black; padding: 8px 12px; border: none; border-radius: 4px; cursor: pointer; transition: background-color 0.3s ease;">না, ধন্যবাদ</button>
        </div>
    `;
